...
public partial class CustomMediaPicker : IMediaPicker
{
    ...
    private class CustomCameraCaptureUI
    {
        ...
        public async Task<StorageFile> CaptureFileAsync(CameraCaptureUIMode mode)
        {
            ...
            var tempLocation = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            //var tempLocation = FileSystem.AppDataDirectory;
            StorageFolder folder = await StorageFolder.GetFolderFromPathAsync(tempLocation);
            var tempFileName = $"capture{extension}";
            var tempFile = await folder.CreateFileAsync(tempFileName, CreationCollisionOption.GenerateUniqueName);
